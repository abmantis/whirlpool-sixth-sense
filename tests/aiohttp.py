...
class AiohttpClientMocker:
    ...
    def create_session(self, loop):
        ...
        object.__setattr__(self.session, "_request", self.match_request)
        return self.session

    async def close_session(self):
        if not self.session:
            return
        logger.info("CLOSEINGGGG")
        ...
        for response in self._mocks:
            if response.match_request(method, url, params):
                ...
                if response.side_effect:
                    response = await response.side_effect(method, url, data, headers)
                if response.exc:
                    raise response.exc
                return response
...
class AiohttpClientMockResponse:
    ...
    def request_info(self):
        """Mock request info"""
        return mock.Mock(real_url=self._url)
