...
from whirlpool.httpapi import appliancesmanager as http_appliancesmanager
...
async def test_keepalive_periodically_fetches_an_appliance(
    ...
    mocks = {
        ...
        for said in http_manager.all_appliances
    }
    kept_alive_mock = mocks[next(iter(http_manager.all_appliances))]

    # Restart the listener so the patched keepalive interval takes effect.
    await http_manager.stop_event_listener()
    await http_manager.start_event_listener()
