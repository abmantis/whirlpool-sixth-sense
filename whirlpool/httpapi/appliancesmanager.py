...
class AppliancesManager:
    ...
    async def _fetch_appliances(self) -> bool:
    ...
    async def connect(self) -> bool:
        ...
        if not await self._fetch_appliances():
            LOGGER.error("Failed to fetch appliances")
            return False

        if not self.all_appliances:
            LOGGER.info("No appliances found")
            return True
