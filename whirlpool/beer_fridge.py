...
class BeerFridge(Appliance):
    ...
    async def set_especific_temp(self, temp: int):
        ...
        if temp not in allowed_temps:
            raise ValueError(f"Invalid temperature: {temp}. Allowed values are {allowed_temps}.")
    ...
    async def set_display_lock(self, display: bool):
        await self.send_attributes({SETTING_DISPLAY_LOCK: self.bool_to_attr_value(display)})
